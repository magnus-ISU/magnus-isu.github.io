...
<div id="cities">
	{#each Object.entries(cities) as city}
		<City
			name={city[0]}
			obelisk={city[1].obelisk}
			walls={city[1].walls}
			barracks={city[1].barracks}
			soldiers={city[1].soldiers}
			bind:action={city[1].action}
			bind:target={city[1].target}
			bind:lore={city[1].lore}
			{cities}
			{actions}
		/>
	{/each}
...
{#if Object.entries(cities).length === 0}
	<button on:click={addCity}> Begin the Obelisk... </button>
{:else}
	<div
		style="display: flex; flex-direction:row; justify-content: center; align-items: center; width: 100%"
	>
		<button on:click={gameTick}> End this Turn </button>
		<button on:click={addCity}> Add another city-state </button>
	</div>
{/if}

<p class="lore">
	King, you must now gather your masons and bolster your defenses, for you have heard the plea of
	the gods. You must build an Obelisk.
</p>
<p class="lore">
	It will scrape the heavens and be immortal in its glory, and your name will be forever carved
...
<div id="rules">
	...
	<ul>
		...
		<li>
			...
			<ul>
				...
				<li>
					If any other player has attacked that player, you lose one soldier for each soldier of the
					attacker with the highest number of soldiers.
				</li>
				<li>You then lose one soldier for each of your target’s Walls.</li>
				<li>
					If your target is not Attacking or Recruiting, you lose one soldier for each of their
					Soldiers. They also lose 1 soldier for each of your soldiers lost this way.
				</li>
				<li>
					For each soldier you have remaining, remove one of your target’s Obelisk. If they reach 0,
					they lose the game.
				</li>
			</ul>
		</li>
	...
	<p>
		You gain a soldier from your Holiday feast immediately. Then attacks are processed. Finally, all
