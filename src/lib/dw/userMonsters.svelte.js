...
export const userMonsters = {
	get list() { return monsters; },
	add(m) { monsters = [...monsters, m]; save(monsters); },
	remove(name) { monsters = monsters.filter(m => m.name !== name); save(monsters); },
	import(incoming) {
		const existing = new Set(monsters.map(m => m.name.toLowerCase()));
		const toAdd = incoming.filter(m => !existing.has(m.name.toLowerCase()));
		if (toAdd.length) { monsters = [...monsters, ...toAdd]; save(monsters); }
	},
	hasName(name) {
